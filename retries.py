...
def evaluate_question(question):
    if "question" not in question:
        ...
        raise Exception("value \"options\" has length less than 1")
    ...
    for i, option in enumerate(question["options"]):
        if type(option) is not str:
            ...
            raise Exception(f"value \"options\"[{i}] has length less than 1")
        elif len(option) > 20:
            raise Exception(f"value \"options\"[{i}] has length greater than 20")
